...
namespace mu2e
{
  ...
  public:
    struct Config
    {
      ...
      // controls whether "value-vs-channel" histos store mean or sum
      fhicl::Atom<std::string> summaryAgg{fhicl::Name("summaryAgg"), "mean"};
  ...
  private:
    ...
    enum class SummaryAgg { Sum, Mean };

    static SummaryAgg parseSummaryAgg(std::string s)
    {
      std::transform(s.begin(), s.end(), s.begin(),
                     [](unsigned char c) { return std::tolower(c); });

      if(s == "sum") return SummaryAgg::Sum;
      return SummaryAgg::Mean;
    }

    void fillSummary(TH1* h, double x, double y)
    {
      if(!h) return;

      if(summaryAgg_ == SummaryAgg::Mean)
        {
          auto* hp = dynamic_cast<TProfile*>(h);
          if(hp) hp->Fill(x, y);
        }
      else
        {
    ...
    TH1* h_baseline_disk0_{nullptr};
    TH1* h_baseline_disk1_{nullptr};

    // h_rms_diskX_: per-disk baseline RMS (noise proxy) vs encoded channel
    TH1* h_rms_disk0_{nullptr};
    TH1* h_rms_disk1_{nullptr};

    // h_maxval_diskX_: per-disk peak ADC (raw, not baseline-subtracted) vs encoded channel
    TH1* h_maxval_disk0_{nullptr};
    TH1* h_maxval_disk1_{nullptr};

    // Global ID distributions for quick sanity checks
    TH1* h_global_channel_dist_{nullptr};  // counts by channel ID [0..19]
    TH1* h_global_board_dist_{nullptr};    // counts by board ID   [0..159]
  ...
  CaloDQMOffline::CaloDQMOffline(const art::EDAnalyzer::Table<Config>& config)
    ...
    , summaryAgg_(parseSummaryAgg(config().summaryAgg()))
    ...
    , diskFormula_(config().diskFormula())
  {
    ...
    // helper to build either TH1F (sum) or TProfile (mean)
    auto makeValueVsEncoded = [&](art::TFileDirectory& dir,
                                  const char* name,
                                  const char* title,
                                  int nb, double x1, double x2,
                                  const char* yTitleMean,
                                  const char* yTitleSum) -> TH1*
    {
      TH1* h = nullptr;
      if(summaryAgg_ == SummaryAgg::Mean)
        {
          h = dir.make<TProfile>(name, title, nb, x1, x2);
          h->GetYaxis()->SetTitle(yTitleMean);
        }
      else
        {
          h = dir.make<TH1F>(name, title, nb, x1, x2);
          h->GetYaxis()->SetTitle(yTitleSum);
        }
      h->SetMarkerStyle(20);
      h->GetXaxis()->SetTitle("Encoded Channel (boardID * 100 + chanID)");
      return h;
    };

    h_baseline_disk0_ = makeValueVsEncoded(*disk0Dir_,
                                           "h_base_d0", "Baseline (Disk 0)",
                                           8020, 0, 8020,
                                           "Mean Baseline [ADC]",
                                           "Sum Baseline [ADC]");

    h_baseline_disk1_ = makeValueVsEncoded(*disk1Dir_,
                                           "h_base_d1", "Baseline (Disk 1)",
                                           7920, 8000, 15920,
                                           "Mean Baseline [ADC]",
                                           "Sum Baseline [ADC]");

    h_rms_disk0_ = makeValueVsEncoded(*disk0Dir_,
                                      "h_rms_d0", "RMS (Disk 0)",
                                      8020, 0, 8020,
                                      "Mean RMS [ADC]",
                                      "Sum RMS [ADC]");

    h_rms_disk1_ = makeValueVsEncoded(*disk1Dir_,
                                      "h_rms_d1", "RMS (Disk 1)",
                                      7920, 8000, 15920,
                                      "Mean RMS [ADC]",
                                      "Sum RMS [ADC]");

    h_maxval_disk0_ = makeValueVsEncoded(*disk0Dir_,
                                         "h_max_d0", "Max ADC (Disk 0)",
                                         8020, 0, 8020,
                                         "Mean Peak ADC [ADC]",
                                         "Sum Peak ADC [ADC]");

    h_maxval_disk1_ = makeValueVsEncoded(*disk1Dir_,
                                         "h_max_d1", "Max ADC (Disk 1)",
                                         7920, 8000, 15920,
                                         "Mean Peak ADC [ADC]",
                                         "Sum Peak ADC [ADC]");
    ...
    for(const auto& digi : caloDigis)
      {
        ...
        fillSummary(disk == 0 ? h_baseline_disk0_ : h_baseline_disk1_, encoded, baseline);
        fillSummary(disk == 0 ? h_rms_disk0_      : h_rms_disk1_,      encoded, rms);
        fillSummary(disk == 0 ? h_maxval_disk0_   : h_maxval_disk1_,   encoded, ampRaw);
        ...
        if(enableBoardHistos_ &&
           (maxBoardHistos_ < 0 || (int)boardHistos_.size() < maxBoardHistos_))
          {
            ...
            if(histos.empty())
              {
                ...
                if(summaryAgg_ == SummaryAgg::Mean)
                  ...
                  histos["base"]->GetYaxis()->SetTitle("Sum Baseline [ADC]");
                  histos["rms"]->GetYaxis()->SetTitle("Sum RMS [ADC]");
                  histos["max"]->GetYaxis()->SetTitle("Sum Peak ADC [ADC]");
                }
            ...
            fillSummary(histos["base"], chanID, baseline);
            fillSummary(histos["rms"], chanID, rms);
            fillSummary(histos["max"], chanID, ampRaw);
